...
const babelPlugins =  process.env.NODE_ENV === 'production'? [
...
module.exports = function(argv) {
    ...
    const prodConfig = resources.createServeConfig({
        ...
        resolve: {
            alias: {
				/**
				 * N.B: 加了些alias当时能解决些重复引包的问题（可能是由于npm link的原因）
				 * 后来发现应该是由于默认的optimization.maxAsyncRequests太小不够用导致的
				 * FIXME: 待验证是否还有加alias的必要
				 */
				'lodash': path.resolve(ROOT_PATH, './node_modules/lodash'),
				// 'echarts': path.resolve(ROOT_PATH, './node_modules/echarts'),
                'mobx': path.resolve(ROOT_PATH, './node_modules/mobx'),
                'mobx-react': path.resolve(ROOT_PATH, './node_modules/mobx-react'),
				// 'mobx-state-tree': path.resolve(ROOT_PATH, './node_modules/mobx-state-tree'),
				'@root': path.resolve(APP_PATH),
                '@main': path.resolve(APP_PATH, './main'),
                '@icons': path.resolve(APP_PATH, './common/icons'),
                // '@client': path.resolve(APP_PATH, './client'),,
